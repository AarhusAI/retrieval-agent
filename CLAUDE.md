...
Several pieces of agent behaviour worth knowing because they are not obvious from any single file:

- **Side-channel results.** Full retrieval results (all metadata) are stashed on `AgentDeps.full_results`; the `retrieve` tool returns only previews to the LLM — text truncated to `AGENT_TOOL_PREVIEW_CHARS`, metadata reduced to `source`. The final pipeline output comes from `full_results`, not from anything the LLM sees. This keeps token usage low on small-context models.
- **Cross-query interleave.** Both the previews (`_build_previews`) and the final payload (`_dedup_results`) merge the per-query result sets **round-robin** via `_interleave_dedup` (position 0 = each query's best — each set is already rerank-sorted — score-descending tiebreak, MD5 dedup), *not* first-query-then-second. This stops one query (e.g. a drifted reformulation that matches document boilerplate) from monopolising a small `k` and starving a relevant chunk another query surfaced. There is deliberately **no** global score re-sort and **no** score floor, so a reranker-underscored-but-relevant chunk stays near the front where a downstream `top_k`/threshold can't silently drop it.
- **Raw-query seed.** With `AGENT_INCLUDE_RAW_QUERY` (default on), `agentic_search` runs one deterministic retrieval pass with the user's *original* query before the agent loop and seeds `full_results`; the agent's own retrievals then append. So the unmodified question is always searched regardless of how the agent rewrites it — guarding recall against query-rewrite drift (e.g. an upstream web-search-style `RETRIEVAL_QUERY_GENERATION_PROMPT_TEMPLATE`). Failures in the seed pass are swallowed; it's a safety-net, not a hard dependency.
...
- **Timeout & accumulation.** `AGENT_TIMEOUT` is wall-clock. On timeout, whatever was already written to `AgentDeps.full_results` (raw-query seed + any completed `retrieve` calls) is returned. Multiple `retrieve` calls (corrective retries) **append** to `full_results`; they don't overwrite.
...
- `AGENT_INCLUDE_RAW_QUERY` (default `true`) — seed retrieval with the user's original query before the agent loop (see the "Raw-query seed" agent behaviour above). Disable only if you fully trust the upstream query generation and want to save one embedding+search per request.
...
- **Reranker fails open**: any HTTP/transport error (status, connect, timeout, protocol) or malformed response body falls back to unranked results — a missing or misbehaving reranker won't take down search. Treat this as deliberate.
- **Reranker fuses, doesn't replace** (`RERANK_FUSION`, default `rrf`): `rerank()` RRF-fuses the retrieval (dense/hybrid) order with the cross-encoder order rather than ordering by cross-encoder score alone, so a strong dense hit the cross-encoder underranks isn't buried out of top-`k`. The returned `distance` is still the cross-encoder relevance score (only the *ordering* is fused). Set `RERANK_FUSION=replace` for the old cross-encoder-only behaviour. Callers must pass documents in retrieval-score order (the input index is treated as the retrieval rank).
